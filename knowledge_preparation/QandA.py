...
def get_QandA_extended(file_path="Q&A.csv"):
    ...
    for column in data.columns[1:]:
        ...
        QandA_extended.append({"pregunta": question, "respuestas": answers})
...
def get_synth_responses(QandA_ext):
    faqs = []
    for item in QandA_ext:
        prompt = {"role": "system", "content": get_synth_template(item)}
        # Assuming the response is in the first element
        response = get_completion_from_messages([prompt])[0]
        faqs.append({"pregunta": item["pregunta"], "respuesta": response})
...
QandA_large = get_QandA_extended("Q&A.csv")
get_synth_responses(QandA_large)
